/**
 * LoadingService - Promise-based batch configuration loading
 * ES5 Compatible - Replaces XMLHttpRequest with fetch polyfill
 */
...
/**
 * Initialize loading with shared key
...
LoadingService.prototype.loadConfig = function(loader) {
  ...
  return this.fetch(url).then(function(response) {
    ...
    // Continue even if one config fails
  });
};

/**
 * Fetch wrapper (uses fetch or XMLHttpRequest fallback)
 * @param {string} url
 * @returns {Promise}
 */
...
/**
 * On progress callback registration
...
/**
 * On complete callback registration
 * @param {Function} callback
 * @returns {Function} Unsubscribe
 */
...
ClientConfigLoader.prototype.resolveTitle = function() {
  // Logic from original ClientConfig
